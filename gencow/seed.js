...
export default async function seed(ctx) {
    ...
    const insertedSites = existingSites.length > 0
        ? existingSites
        : await ctx.db
            .insert(sites)
            .values([
            ...
            .returning();
    const sayuwonSite = insertedSites.find((s) => s.name === "사유원");
    const yeoyongkukSite = insertedSites.find((s) => s.name === "여용국한방스파");
    const highOneSite = insertedSites.find((s) => s.name === "하이원리조트");
    const existingEbooks = await ctx.db.select().from(ebooks);
    const insertedEbooks = existingEbooks.length > 0
        ? existingEbooks
        : await ctx.db
            .insert(ebooks)
            .values([
            ...
            .returning();
    const arirangEbook = insertedEbooks.find((e) => e.title.includes("Huntr/x"));
    const koreanLangEbook = insertedEbooks.find((e) => e.title.includes("Korean Language"));
